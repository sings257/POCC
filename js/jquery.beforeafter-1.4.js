...
(function($){
	$.fn.extend({ 
		...
		var randID =  Math.round(Math.random()*100000000);
			...
			$(obj).prepend('<div id="dragwrapper'+randID+'"><div id="drag'+randID+'"><img width="80" height="80" alt="handle" src="'+o.imagePath+'handle.png" id="handle'+randID+'" position="relative" /></div></div>'); // Create drag handle
			...
			$('div:eq(2)', obj).height(imgHeight).width(50).css({'position':'absolute','overflow':'hidden','left':'0px','z-index':'10'}); // Set CSS properties of the before image div
			...
			$('#drag'+randID).width(7).height(imgHeight).css({'background':o.dividerColor,'position':'absolute','left':'3px'});	// Set drag handle CSS properties
			$('#beforeimage'+randID).css({'position':'absolute','top':'0px','left':'0px'});
			$('#afterimage'+randID).css({'position':'absolute','top':'0px','right':'0px'});
			$('#handle'+randID).css({'opacity': '1','z-index':'100','position':'relative','cursor':o.cursor,'top':(imgHeight/2)-($('#handle'+randID).height()/2)+'px','left':'-37px'});
			...
			function clickit()
			{
				$(obj).hover(function(){
						...
						$('#lt-arrow'+randID).animate({opacity:1,left:parseInt($('#lt-arrow'+randID).css('left'))-6+'px'},350);
						$('#rt-arrow'+randID).animate({opacity:1,left:parseInt($('#rt-arrow'+randID).css('left'))+6+'px'},350);
				...
				$('#dragwrapper'+randID).stop().animate({'opacity':1},500);});
